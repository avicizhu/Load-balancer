...
//                                                    192.168.1.1 203.82.48.1     203.82.48.2
  ...
  for(int i = 0; i < 24; i++){
  clientApps4.Add (clientHelper.Install (p2pNodes.Get (1)) );
  Ptr< Application > client = clientApps4.Get(i);
  client->SetStartTime(Seconds (time4));
  client->SetStopTime(Seconds (100.0));
  time4 = time4 + 0.001;
  }
 */
...
// Prepare to run the simulation
  ...
  Simulator::Schedule(Seconds(2.1),&TraceCwnd);
