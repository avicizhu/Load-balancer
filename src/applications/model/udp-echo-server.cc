...
namespace ns3 {
int LBtable[4] = {0,0,0,0};

//extern std::map <uint32_t, int> mapServer; 
...
//          NS_LOG_INFO ("At time " << Simulator::Now ().GetSeconds () << "s server received " << packet->GetSize () << " bytes from " <<
 //                      Ipv4Address(InetSocketAddress::ConvertFrom (from).GetIpv4 ().Get()) << " port " <<
...
//	Ipv4Nat::setServerStatus( InetSocketAddress::ConvertFrom (from).GetIpv4().Get(), 1 );
	
	if( (strncmp(reinterpret_cast<const char*>(buf),"close",5) == 0) ){
	//received connection closing signal, conn num minus one	
	Ipv4Nat::setServerStatus( InetSocketAddress::ConvertFrom (from).GetIpv4().Get(), 0 );
